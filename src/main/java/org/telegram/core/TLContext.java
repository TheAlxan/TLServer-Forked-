...
import org.telegram.tl.StreamUtils;
...
public class TLContext {
    ...
    public final <T extends TLObject> T deserializeMessage(InputStream stream, Class<T> clazz, int constructorId) throws Exception {
        int realConstructorId = StreamUtils.readInt(stream);
        if (constructorId != -1 && realConstructorId != constructorId) {
            throw new Exception(""+realConstructorId+" :: "+ constructorId);
        ...
        try {
            ...
            message.deserialize(new ProtocolBuffer(1));
            return message;
